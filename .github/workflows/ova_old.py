...
class OVAStrategy(ConnectionStrategy):
    ...
    def install_dependencies(self) -> bool:
        ...
        try:
            ...
            self.allocator_connection.execute_command(
                f"rm -rf /tmp/wazuh-virtual-machines && {cmd} /tmp/wazuh-virtual-machines && cd /tmp/wazuh-virtual-machines && git checkout main"
            )
    ...
    def download_ova_from_s3(self) -> bool:
        """Download OVA file from S3 using a pre-signed URL, then copy to the allocator instance.
        ...
        try:
            ...
            logger.info(f"Generating pre-signed URL for S3 object: {self.config.ova_s3_path}")
            ...
            success = run_with_progress(
                self._download_from_presigned_url,
                args=(presigned_url, local_ova_path),
                duration_seconds=30,
                operation_name="S3 Download (Pre-signed URL)",
            )
    ...
    def _generate_presigned_url(self, bucket_name: str, key: str, expiration: int = 3600) -> str | None:
        ...
        try:
            ...
            from ..aws.credentials import AWSCredentialsManager, AWSRole

            bucket_region = "us-west-1"

            # Obtener credenciales temporales mediante role assumption
            aws_role = AWSRole(self.config.aws_role)
            credentials_manager = AWSCredentialsManager()
            profile, role_arn = credentials_manager.get_credentials(aws_role)
            session = credentials_manager.create_session(profile, role_arn, bucket_region)

            # Obtener las credenciales temporales de la sesión
            credentials = session.get_credentials().get_frozen_credentials()

            logger.debug(f"Using credential type: {'temporary (ASIA)' if credentials.token else 'static (AKIA)'}")

            # Crear cliente S3 con virtual-hosted-style explícito
            ...
            logger.info(f"Pre-signed URL generated for {bucket_name}/{key} (expires in {expiration}s)")
    ...
    def _download_from_presigned_url(self, url: str, local_path: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Starting download from pre-signed URL to {local_path}")
                        ...
                        if chunk:
                            ...
                            if total_size > 0:
                                ...
                                if downloaded % (chunk_size * 10) == 0:
                                    logger.debug(f"Download progress: {progress:.1f}% ({downloaded / (1024**3):.2f} GB)")

            logger.info(f"Download completed successfully: {local_path}")
            return True

        except Exception as e:
            logger.error(f"Error downloading from pre-signed URL: {str(e)}")
            logger.error(f"Trace: {traceback.format_exc()}")
            return False