...
def set_authd_password() -> None:
    ...
    the manager. It also removes the agent key baked into the image so the agent re-enrolls with the
    rotated password instead of reusing the shared key created during the build.
    ...
    rm -f {WAZUH_AGENT_CLIENT_KEYS_FILE}
    """
    _, error_output = exec_command(command=command)
    if error_output:
