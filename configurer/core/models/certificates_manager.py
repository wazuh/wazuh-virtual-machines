...
class CertsManager:
    def __init__(
        self, raw_config_path: Path, certs_tool_path: Path, client: paramiko.SSHClient | None = None
    ) -> None:
    ...
    def _get_certs_name(self, component: Component, component_config_file: str, flattened_key: bool = True, client: paramiko.SSHClient | None = None):
        certs_name = {}
        ...
        for component in Component:
            if component != Component.ALL:
                certs_name = self._get_certs_name(
                    component=component,
                    component_config_file=ComponentConfigFile.WAZUH_INDEXER if component == Component.WAZUH_INDEXER else ComponentConfigFile.WAZUH_SERVER if component == Component.WAZUH_SERVER else ComponentConfigFile.WAZUH_DASHBOARD,
                ...
                output, error_output = self.copy_certs_to_component_directory(
                    component=component,
                    certs_name=certs_name,
                    client=client
                )
                
                if error_output:
                    logger.error(f"Error while copying certificates to {component.replace('_', ' ')} directory")
                    raise Exception(f"Error while copying certificates to {component.replace('_', ' ')} directory: {error_output}")
            
        logger.info_success("Certificates generated successfully")
        
        
    def copy_certs_to_component_directory(self, component: Component, certs_name: dict, client: paramiko.SSHClient | None = None) -> tuple[str, str]:
        ...
        if component == Component.WAZUH_INDEXER:
            ...
            command = f"""
                ...
                sudo tar -xvf {RemoteDirectories.CERTS}/wazuh-certificates.tar -C {ComponentCertsDirectory.WAZUH_SERVER} ./{" ./".join(self.components_certs_default_name[Component.WAZUH_SERVER].values())}
