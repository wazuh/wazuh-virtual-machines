...
def parse_args() -> argparse.Namespace:
    ...
    ova_group.add_argument(
        "--allocator-instance-type", default="t3.xlarge",
...
def load_config_from_args(args: argparse.Namespace) -> BaseTesterConfig:
    ...
    if test_type == TestType.OVA and hasattr(args, 'ova_s3_path') and args.ova_s3_path:
        ...
        config = OVATesterConfig(
            ...
            allocator_instance_type=args.allocator_instance_type if hasattr(args, 'allocator_instance_type') else "metal",
...
def run_tests(config: BaseTesterConfig, args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.test_pattern:
            ...
            test_patterns = config.test_patterns.get(config.test_type, ["*"])
...
#==================================================
# Archivo Python: /home/fcaffieri/wazuh-env/wazuh-virtual-machines/wazuh_vm_tester/src/vm_tester/tests/test_ova.py
#==================================================
...
#==================================================
# Archivo Python: /home/fcaffieri/wazuh-env/wazuh-virtual-machines/wazuh_vm_tester/src/vm_tester/strategies/ova.py
#==================================================
...
class OVAStrategy(ConnectionStrategy):
    ...
    def setup_allocator_instance(self) -> bool:
        ...
        try:
            ...
            instance_name = f"gha_{os.environ.get('GITHUB_RUN_ID', str(int(time.time())))}_ova_build"
            composite_name = f"ova_test_{int(time.time())}"
    ...
    def install_dependencies(self) -> bool:
        ...
        try:
            ...
            self.allocator_connection.execute_command("which git || sudo apt-get update && sudo apt-get install -y git")
            self.allocator_connection.execute_command("which pip3 || sudo apt-get install -y python3-pip")

            # Clone the repository directly on the remote machine
            logger.info("Cloning wazuh-virtual-machines repository")
            self.allocator_connection.execute_command("rm -rf /tmp/wazuh-virtual-machines && git clone https://github.com/wazuh/wazuh-virtual-machines.git /tmp/wazuh-virtual-machines")

            # Install required Python packages on remote
            self.allocator_connection.execute_command("pip3 install requests")

            # Create a simple script to import and run the dependencies installer
            script_content = """
                import sys
                import os

                # Add the repository to Python path
                sys.path.insert(0, '/tmp/wazuh-virtual-machines')

                try:
                    from configurer.ova.ova_pre_configurer.install_dependencies import main as install_dependencies_main
                    print("Running install_dependencies_main()...")
                    install_dependencies_main()
                    print("Dependencies installation completed successfully")
                    sys.exit(0)
                except Exception as e:
                    print(f"Error importing or running install_dependencies_main: {e}")
                    import traceback
                    traceback.print_exc()
                    sys.exit(1)
            """
            ...
            script_path = "/tmp/install_deps.py"
            cmd = f'cat > {script_path} << EOL\n{script_content}\nEOL\nchmod +x {script_path}'
            self.allocator_connection.execute_command(cmd)

            # Execute the script
            logger.info("Running dependencies installation script...")
            exit_code, stdout, stderr = self.allocator_connection.execute_command(f"python3 {script_path}")
    ...
    def port_forward(self) -> bool:
        ...
        try:
            ...
            logger.info("Setting up SSH port forwarding (22 -> 2222)")
            exit_code, stdout, stderr = self.allocator_connection.execute_command(
                f'VBoxManage modifyvm "{self.vm_name}" --natpf1 "ssh,tcp,,2222,,22"'
            )
    ...
    def create_connection(self) -> Optional[ConnectionInterface]:
        ...
        # Otherwise, start the OVA import and testing process
        ...
        if not self.setup_allocator_instance():
            ...
            self.cleanup()
            raise RuntimeError("Allocator instance setup failed")
        ...
        try:
            ...
            connection = SSHConnection(
                ...
                host=self.instance_public_ip,  # Connect to the allocator instance
                username=self.config.vm_username or "wazuh-user",
                port=2222,  # The forwarded port we configured
                key_path=self.config.ssh_key_path,
                private_key=self.config.ssh_private_key
            )
        ...
        if self.config.terminate_on_completion:
            try:
                ...
                if os.path.exists(track_file):
                    delete_cmd = f"python3 {wazuh_automation_path}/deployability/modules/allocation/main.py --action delete --track-input {track_file}"
...
==================================================
Resumen:
Total de archivos Python procesados: 40
==================================================
