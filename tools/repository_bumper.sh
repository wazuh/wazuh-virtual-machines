...
update_version_in_files() {
    ...
    for file in "${m_m_p_files[@]}"; do
        sed -i -E "s/\bv${OLD_MAJOR}\.${OLD_MINOR}\.${OLD_PATCH}\b/v${NEW_MAJOR}.${NEW_MINOR}.${NEW_PATCH}/g; s/(^|[^0-9])${OLD_MAJOR}\.${OLD_MINOR}\.${OLD_PATCH}([^0-9]|$)/\1${NEW_MAJOR}.${NEW_MINOR}.${NEW_PATCH}\2/g" "${file}"
