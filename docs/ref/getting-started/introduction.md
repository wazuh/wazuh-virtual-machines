...
Wazuh provides the option to deploy two types of virtual machines with a base all-in-one (AIO) installation that includes all the Wazuh components: the Wazuh Server, Wazuh Indexer, Wazuh Dashboard, and Wazuh Agent. This allows us to have a ready-to-use virtual machine without needing to configure any components, making it completely transparent to the user.
