...
```bash
curl -sO https://packages.wazuh.com/production/1.x/vm/wazuh-1.2.3-1.ova
```

To use `pre-release` packages instead, use the following command:

```bash
curl -sO https://packages-staging.xdrsiem.wazuh.info/pre-release/1.x/vm/wazuh-1.2.3-<STAGE>.ova
```
