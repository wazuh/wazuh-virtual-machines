...
```bash
curl -sO https://packages.wazuh.com/production/0.x/vm/wazuh-0.0.1-1.ova
```

To use `pre-release` packages instead, use the following command:

```bash
curl -sO https://packages-staging.xdrsiem.wazuh.info/pre-release/0.x/vm/wazuh-0.0.1-<STAGE>.ova
```
