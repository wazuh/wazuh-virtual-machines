...
@pytest.mark.parametrize(
    ...
    [
        (Component.WAZUH_INDEXER, Package_type.RPM, {
            Component_arch.X86_64: "http://packages.wazuh.com/indexer/x86_64.rpm",
            Component_arch.AARCH64: "http://packages.wazuh.com/indexer/aarch64.rpm",
        }),
        (Component.WAZUH_SERVER, Package_type.RPM, {
            Component_arch.X86_64: "http://packages.wazuh.com/server/x86_64.rpm",
            Component_arch.AARCH64: "http://packages.wazuh.com/server/aarch64.rpm",
        }),
        (Component.WAZUH_DASHBOARD, Package_type.RPM, {
            Component_arch.X86_64: "http://packages.wazuh.com/dashboard/x86_64.rpm",
            Component_arch.AARCH64: "http://packages.wazuh.com/dashboard/aarch64.rpm",
        }),
    ]
...
@pytest.mark.parametrize(
    ...
    [
        (Component.WAZUH_INDEXER, Package_type.RPM, Component_arch.X86_64, AnyUrl("http://packages.wazuh.com/indexer/x86_64.rpm")),
        (Component.WAZUH_INDEXER, Package_type.RPM, Component_arch.AARCH64, AnyUrl("http://packages.wazuh.com/indexer/aarch64.rpm")),
        (Component.WAZUH_SERVER, Package_type.RPM, Component_arch.X86_64, AnyUrl("http://packages.wazuh.com/server/x86_64.rpm")),
        (Component.WAZUH_SERVER, Package_type.RPM, Component_arch.AARCH64, AnyUrl("http://packages.wazuh.com/server/aarch64.rpm")),
        (Component.WAZUH_DASHBOARD, Package_type.RPM, Component_arch.X86_64, AnyUrl("http://packages.wazuh.com/dashboard/x86_64.rpm")),
        (Component.WAZUH_DASHBOARD, Package_type.RPM, Component_arch.AARCH64, AnyUrl("http://packages.wazuh.com/dashboard/aarch64.rpm")),
    ]
)
...
def test_get_package_by_arch_invalid_url(package_info, component, package_type, component_arch):
    ...
    with pytest.raises(ValueError, match=f"URL for {component.value} with {component_arch.value} architecture has an invalid format."):
...
def test_get_package_by_arch_missing_arch(package_info, component, package_type, component_arch):
    ...
    with pytest.raises(ValueError, match=f"Arch {component_arch.value} not found in {component.value} packages. Expected an URL but got None."):
        package_info.get_package_by_arch(component, package_type, component_arch)
...
def test_get_package_by_arch_invalid_host(package_info, component, package_type, component_arch):
    ...
    with pytest.raises(ValueError, match=f"URL for {component.value} with {component_arch.value} architecture is not for Wazuh packages."):
        package_info.get_package_by_arch(component, package_type, component_arch)
