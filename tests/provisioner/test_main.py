...
def test_parse_arguments_required():
    test_args = [
        ...
        "--packages-url-path", "packages_url.yaml"
    ]
    sys.argv = test_args
...
def test_parse_arguments_optional():
    test_args = [
        ...
        "--package-type", "deb",
        "--arch", "arm64",
        "--dependencies", "custom_dependencies.yaml",
...
@pytest.mark.parametrize(
    ...
    [
        ("rpm", "x86_64", "http://example.com/package.rpm", ["dependency1", "dependency2"], Component.WAZUH_SERVER),
...
@pytest.mark.parametrize(
    ...
    [
        (Component.ALL, 
         [ComponentInfo(name=Component.WAZUH_INDEXER, package_url=AnyUrl("http://example.com/all.rpm"), dependencies=["dependency1", "dependency2"]), 
          ComponentInfo(name=Component.WAZUH_SERVER, package_url=AnyUrl("http://example.com/all.rpm"), dependencies=["dependency1", "dependency2"]),
          ComponentInfo(name=Component.WAZUH_DASHBOARD, package_url=AnyUrl("http://example.com/all.rpm"), dependencies=["dependency1", "dependency2"])
        ],
            "rpm", 
            "x86_64"
         ),
        (Component.WAZUH_INDEXER, 
         [ComponentInfo(name=Component.WAZUH_INDEXER, package_url=AnyUrl("http://example.com/indexer.rpm"), dependencies=["dependency1", "dependency2"])],
            "rpm",
            "x86_64"
         ),
        (Component.WAZUH_SERVER, 
         [ComponentInfo(name=Component.WAZUH_SERVER, package_url=AnyUrl("http://example.com/server.rpm"), dependencies=["dependency3", "dependency4"])],
            "rpm",
            "x86_64"
         ),
        (Component.WAZUH_DASHBOARD, 
         [ComponentInfo(name=Component.WAZUH_DASHBOARD, package_url=AnyUrl("http://example.com/dashboard.deb"), dependencies=["dependency5", "dependency6"])],
            "deb",
            "arm64"
         ),
    ],
...
def test_parse_componets(component, expected_components, package_type, arch):
    ...
    for res in result:
        ...
        assert next(res for arg in mock_input.packages_url_content.get_package_by_arch.call_args_list 
                    if arg.kwargs["component"] == res.name 
                    and arg.kwargs["package_type"] == package_type 
                    and arg.kwargs["component_arch"] == arch)
        assert next(res for arg in mock_input.dependencies.get_component_dependencies.call_args_list if arg.kwargs["component"] == res.name)
