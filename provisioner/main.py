...
def parse_arguments():
    ...
    parser.add_argument(
        "--inventory", required=True, help="Path to the inventory file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dependencies", required=False, default=DEPENDENCIES_FILE_PATH, help="Path to the dependencies file"
    )
    parser.add_argument(
        "--component", required=False, default="all", choices=["wazuh_indexer", "wazuh_server", "wazuh_dashboard", "all"], help="Component to provision"
    )
...
def get_component_info(input: Input, component: Component) -> ComponentInfo:
    return ComponentInfo(
        name=component,
        package_url=input.packages_url_content.get_package_by_arch(component=component, package_type=input.package_type, component_arch=input.arch),
...
def main():
    ...
    Provisioner(inventory=input.inventory_content, certs=input.certificates_content, components=components, arch=input.arch, package_type=input.package_type).provision()
