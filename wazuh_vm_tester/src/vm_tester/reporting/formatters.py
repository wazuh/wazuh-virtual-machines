...
from .base import COLOR_RESET, TestStatus, TestSummary, get_status_color
...
class MarkdownFormatter(ReportFormatter):
    ...
    def format_report(self, summary: TestSummary) -> str:
        ...
        markdown = "# Wazuh VM Test Results\n\n"
                ...
                for test in failed_tests:
                    ...
                    if test.message:
                        ...
                        if self.debug_mode:
                            ...
                            if "\n" in processed_message and any(
                                line.startswith("- ") for line in processed_message.split("\n")
                            ):
...
class ConsoleFormatter(ReportFormatter):
    ...
    def format_report(self, summary: TestSummary) -> str:
        ...
        output.append(
            f"Passed: {get_status_color(TestStatus.PASS, self.use_colors)}{summary.passed}{self._get_color('RESET')}"
        )
        output.append(
            f"Failed: {get_status_color(TestStatus.FAIL, self.use_colors)}{summary.failed}{self._get_color('RESET')}"
        )
        output.append(
            f"Warnings: {get_status_color(TestStatus.WARNING, self.use_colors)}{summary.warnings}{self._get_color('RESET')}"
        )
        output.append(
            f"Errors: {get_status_color(TestStatus.ERROR, self.use_colors)}{summary.errors}{self._get_color('RESET')}"
        )
        output.append(
            f"Skipped: {get_status_color(TestStatus.SKIPPED, self.use_colors)}{summary.skipped}{self._get_color('RESET')}"
        )
                    ...
                    if test.message:
                        ...
                        if self.debug_mode:
                            ...
                            if "\n" in processed_message:
                                ...
                                message = (
                                    processed_message[:150] + "..."
                                    if len(processed_message) > 150
        ...
        if summary.warnings > 0 or summary.skipped > 0:
            ...
            for module, tests in tests_by_module.items():
                ...
                for test in other_tests:
                    status_color = get_status_color(test.status, self.use_colors)
                    status_symbol = f"{status_color}⚠" if test.status == TestStatus.WARNING else f"{status_color}○"
                    ...
                    if test.message:
                        ...
                        if self.debug_mode and test.status == TestStatus.WARNING:
                            ...
                            if "\n" in processed_message:
                                for line in processed_message.split("\n"):
                                    output.append(
                                        f"    {self._get_color('CYAN')}→ {line[:150]}{self._get_color('RESET')}"
                                    )
                            else:
                                output.append(
                                    f"    {self._get_color('CYAN')}→ {processed_message[:150]}{self._get_color('RESET')}"
                                )
