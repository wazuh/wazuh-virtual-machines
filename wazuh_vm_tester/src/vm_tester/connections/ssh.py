...
class SSHConnection(ConnectionInterface):
    ...
    def connect(
        ...
        **kwargs,
        ...
        logger.info(
            f"Attempting to establish SSH connection to {self._host}:{self._port} (max {max_retries} attempts, timeout {timeout}s per attempt)"
        )
        ...
        while attempts < max_retries:
            ...
            logger.info(
                f"SSH connection attempt {attempts}/{max_retries} (auth: {'password' if using_password else 'key'}, port: {self._port})"
            )
    ...
    def execute_command(
        ...
        while attempt < max_attempts:
            ...
            try:
                ...
                logger.info("Will retry after execution error")
...
class SubprocessSSHClient:
    ...
    def __init__(self, host: str, port: int, username: str, password: str, sshpass_path: str, timeout: int = 30):
                ...
                if process is not None and process.poll() is None:
                    ...
                    try:
                        ...
                        if process.poll() is None:
                            logger.warning("Command didn't respond to SIGTERM, sending SIGKILL")
                            process.kill()
                    except Exception as e:
