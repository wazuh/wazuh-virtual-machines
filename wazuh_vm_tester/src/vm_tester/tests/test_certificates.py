...
class TestCertificates:
    ...
    def test_certificates_exist(self, config: AMITesterConfig):
        ...
        if missing_certificates:
            raise AssertionError("One or more certificates do not exist. " + message)
        else:
    ...
    def test_certificates_validity(self, config: AMITesterConfig):
        ...
        for cert_config in config.certificates:
            ...
            exit_code, stdout, stderr = connection.execute_command(f"openssl x509 -in {cert_path} -noout -checkend 0")
            ...
            try:
                ...
                if days_remaining < cert_config.days_valid:
                    check_result += (
                        f" will expire in {days_remaining} days (less than required {cert_config.days_valid} days)"
                    )
                    invalid_certificates.append(check_result)
                else:
                    check_result += (
                        f" is valid with {days_remaining} days remaining (requirement: {cert_config.days_valid} days)"
                    )
        ...
        if invalid_certificates:
            raise AssertionError("One or more certificates are invalid or expiring soon. " + message)
        else:
    ...
    def test_certificate_subjects(self, config: AMITesterConfig):
        ...
        for cert_config in config.certificates:
            ...
            exit_code, stdout, stderr = connection.execute_command(f"openssl x509 -in {cert_path} -noout -subject")
        ...
        if mismatched_subjects:
            raise AssertionError("One or more certificate subjects do not match expected patterns. " + message)
        else:
    ...
    def test_certificate_issuers(self, config: AMITesterConfig):
        ...
        for cert_config in config.certificates:
            ...
            exit_code, stdout, stderr = connection.execute_command(f"openssl x509 -in {cert_path} -noout -issuer")
        ...
        if mismatched_issuers:
            raise AssertionError("One or more certificate issuers do not match expected patterns. " + message)
        else:
    ...
    def test_certificate_permissions(self, config: AMITesterConfig):
        ...
        for cert_config in config.certificates:
            ...
            exit_code, stdout, _ = connection.execute_command(f"stat -c '%a' {cert_path}")
        ...
        if mismatched_permissions:
            raise AssertionError("One or more certificate permissions do not match expected values. " + message)
        else:
