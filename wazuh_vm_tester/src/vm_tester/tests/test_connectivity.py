...
class TestConnectivity:
    ...
    def test_service_connectivity(self, config: AMITesterConfig):
        ...
        for test in config.connectivity_tests:
            ...
            exit_code, stdout, stderr = connection.execute_command(f"curl -v telnet://{test.host}:{test.port} 2>&1")
        ...
        if failed_connections:
            raise AssertionError("One or more connectivity tests failed. " + message)
        else:
    ...
    def test_wazuh_api_connectivity(self, config: AMITesterConfig):
            ...
            for endpoint_config in service.api_endpoints:
                ...
                if token:
                    ...
                    if exit_code == 0 and stdout.strip():
                        ...
                        endpoint_check_result += " - token obtained successfully"
                    else:
                        endpoint_check_result += f" - failed to obtain token: {stderr}"

                api_command = "curl -s -k "
        ...
        if failed_api_connections:
            raise AssertionError("One or more API connectivity tests failed. " + message)
