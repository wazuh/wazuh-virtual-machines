...
class TestLogs:
    ...
    def test_log_files_exist(self, config: AMITesterConfig):
        ...
        if missing_logs:
            raise AssertionError("One or more log files do not exist. " + message)
        else:
    ...
    def test_logs_for_errors(self, config: AMITesterConfig):
        ...
        for service_config in config.services:
            ...
            for log_command in service_config.log_commands:
                ...
                command = log_command if "grep" in log_command else f"{log_command} | grep -E '{error_patterns_str}'"
        ...
        if logs_with_errors:
            raise AssertionError("Errors found in one or more logs. " + message)
        else:
    ...
    def test_recent_logs(self, config: AMITesterConfig):
        ...
        for service_config in config.services:
            ...
            for log_file in service_config.log_files:
                ...
                command = f"find {log_file} -mtime -1 -type f -exec grep -l -E '{'|'.join(error_patterns)}' {{}} \\;"
