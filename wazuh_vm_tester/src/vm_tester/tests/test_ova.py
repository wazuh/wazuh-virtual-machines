...
from ..config import BaseTesterConfig
...
class TestOVA:
    ...
    def test_residual_files(self, config: BaseTesterConfig):
        ...
        if missing_files:
            raise AssertionError("One or more residual installation files do not exist. " + message)
        else:
    ...
    def test_dns_resolution(self, config: BaseTesterConfig):
        ...
        exit_code, stdout, stderr = connection.execute_command(f"ping -c 1 {test_domain}")
