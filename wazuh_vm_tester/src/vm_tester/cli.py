...
from .reporting.base import COLOR_RESET, TestStatus, get_status_color
...
def parse_args() -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(description="Tool for validating Wazuh VMs")

    # New test type parameter
    parser.add_argument(
        "--test-type", choices=["ami", "ova", "kubernetes", "puppet"], help="Type of test to run (default: ami)"
    )

    # Connection methods - all in mutually exclusive group
    connection_group = parser.add_mutually_exclusive_group(required=True)
    connection_group.add_argument("--ami-id", help="ID of the AMI to validate by launching a new EC2 instance")
    connection_group.add_argument("--ova-s3-path", help="S3 path to the OVA file to import and test")
    connection_group.add_argument("--inventory", help="Path to Ansible inventory file to use for connection details")
    connection_group.add_argument("--ssh-host", help="SSH host to connect to (direct SSH mode)")
    connection_group.add_argument("--use-local", action="store_true", help="Use local machine for testing")

    # For ssh connection
    ssh_group = parser.add_argument_group("Direct SSH Options")
    ssh_group.add_argument("--ssh-username", default="wazuh-user", help="SSH username (default: wazuh-user)")
    ssh_group.add_argument("--ssh-key-path", help="Path to the SSH private key")
    ssh_group.add_argument("--ssh-password", help="Paswword for SSH connection")
    ...
    ansible_group.add_argument(
        "--host", help="Host ID in the Ansible inventory to use (defaults to first host if not specified)"
    )

    # For AMI creation
    aws_group = parser.add_argument_group("AWS Options")
    aws_group.add_argument("--aws-region", default="us-east-1", help="AWS region (default: us-east-1)")
    aws_group.add_argument("--instance-type", default="c5ad.xlarge", help="EC2 instance type (default: c5ad.xlarge)")
    aws_group.add_argument("--subnet-id", help="ID of the subnet where to launch the instance")
    aws_group.add_argument("--instance-profile", help="IAM instance profile name")
    aws_group.add_argument("--no-terminate", action="store_true", help="Do not terminate the instance after tests")
    aws_group.add_argument(
        "--security-group-ids", nargs="+", help="Security group IDs (overrides default security groups)"
    )
    aws_group.add_argument(
        "--aws-role", choices=["qa", "dev", "default"], default="default", help="AWS role to assume (default: default)"
    )

    # For OVA testing
    ova_group = parser.add_argument_group("OVA Options")
    ova_group.add_argument("--allocator-instance-type", help="EC2 instance type for the allocator (default: c5.metal)")
    ova_group.add_argument(
        "--vm-memory", type=int, default=4096, help="Memory to allocate to the VM in MB (default: 4096)"
    )
    ova_group.add_argument("--vm-cpus", type=int, default=2, help="Number of CPUs to allocate to the VM (default: 2)")
    ova_group.add_argument("--import-only", action="store_true", help="Only import the OVA, don't run tests")
    ova_group.add_argument("--vm-username", default="wazuh-user", help="VM username (default: wazuh-user)")
    ova_group.add_argument("--vm-password", default="wazuh", help="VM password (default: wazuh)")

    # Validation parameters
    validation_group = parser.add_argument_group("Validation Options")
    validation_group.add_argument("--version", help="Expected Wazuh version")
    validation_group.add_argument("--revision", help="Expected AMI revision")

    # Logging options
    logging_group = parser.add_argument_group("Logging Options")
    logging_group.add_argument(
        "--log-level", choices=["DEBUG", "INFO", "TRACE"], default="INFO", help="Set logging level (default: INFO)"
    )
    ...
    output_group.add_argument(
        "--output",
        choices=["json", "markdown", "console", "github"],
    )
    output_group.add_argument("--output-file", help="File where to save the results")
    ...
    pytest_group.add_argument("--pytest-args", help="Additional arguments to pass to pytest")
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if (
        args.test_type == "ova"
        and not args.ova_s3_path
        and not args.ssh_host
        and not args.inventory
        and not args.use_local
    ):
    ...
    elif args.inventory and not os.path.exists(args.inventory):
...
def load_config_from_args(args: argparse.Namespace) -> BaseTesterConfig:
    ...
    if test_type == TestType.OVA and hasattr(args, "ova_s3_path") and args.ova_s3_path:
        ...
        config = OVATesterConfig(
            ...
            allocator_instance_type=args.allocator_instance_type
            if hasattr(args, "allocator_instance_type") and args.allocator_instance_type
            else "metal",
...
def run_tests(config: BaseTesterConfig, args: argparse.Namespace) -> int:
    ...
    try:
        ...
        report_manager = ReportManager(
            debug_mode=debug_mode, test_type=config.test_type.value if hasattr(config, "test_type") else None
        )
        ...
        logger.info(
            f"Tests summary: Total: {summary.total}, "
        ...
        failed_tests = [
            t for t in report_manager.results if t.status == TestStatus.FAIL or t.status == TestStatus.ERROR
        ]
